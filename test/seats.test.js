...
import { showSeat, bookSeat } from "../modules/seats.js?db=test";
...
Deno.test("book seats", async () => {
  ...
  console.log(test);assertEquals(test, "edited seats a1 state for show id 1", "book seats fail");
});

Deno.test("book seats no input", async () => {
  const seats = { id: "1", movieId: "1" }
  const test = await bookSeat(seats);
  console.log(test);assertEquals(test, "no changes were made", "book seats no input fail");
});
